...
-- ============================================================================
-- Same fix, applied to "requests" (📥 Request Here admin tab) and
-- "support_messages" (✉️ Messages admin tab) — both were already being
-- written to by the site's floating widget, but the admin panel had no UI
-- to read them until now, and very likely hits the exact same "RLS enabled,
-- no SELECT/DELETE policy" problem as link_alerts did.
-- ============================================================================

alter table public.requests enable row level security;

drop policy if exists "Anyone can insert a request" on public.requests;
...
drop policy if exists "Signed-in users can delete requests" on public.requests;
create policy "Signed-in users can delete requests"
  on public.requests
  for delete
  to authenticated
  using (true);
...
drop policy if exists "Signed-in users can delete messages" on public.support_messages;
create policy "Signed-in users can delete messages"
  on public.support_messages
  for delete
  to authenticated
  using (true);
